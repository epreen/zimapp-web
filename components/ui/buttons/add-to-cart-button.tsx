...
import { useCartActions, useCartItem } from "@/components/providers/cart-store-provider";
...
export function AddToCartButton({
  ...
  const handleAdd = () => {
    if (quantityInCart < stock) {
      addItem({ productId, name, price, slug, image }, 1);
  ...
  if (quantityInCart === 0) {
    return (
      <Button onClick={handleAdd} className={cn("h-11 w-full text-background", className)}>
  ...
  return (
    <div
      className={cn(
        "flex h-11 w-full items-center rounded-md border border-foreground/10",
        className,
      )}
    >
      <Button
