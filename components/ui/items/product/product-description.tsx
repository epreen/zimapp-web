...
const ProductDescription: FC<ProductDescriptionInterface> = ({ product }) => {
    ...
    return (
        <div className="my-18 text-sm text-foreground/80">

            {/* Tabs */}
            <div className="flex border-b border-foreground/20 mb-6 max-w-2xl">
                    ...
                    {product.rating.map((item,index) => (
                        <div key={index} className="flex gap-5 mb-10">
                            ...
                            <div>
                                ...
                                <p className="font-medium text-foreground/80">{item.user.name}</p>
            ...
            <div className="flex gap-3 mt-14">
                <Image src={product.store.logo} alt="" className="size-11 rounded-full ring ring-foreground/40" width={100} height={100} />
                <div>
                    <p className="font-medium text-foreground">Product by {product.store.name}</p>
                    <Link href={`/marketplace/shops/${product.store.username}`} className="flex items-center gap-1.5 text-primary dark:text-secondary"> view store <ArrowRight size={14} /></Link>
                </div>
            </div>
        </div>
    )
