...
function ProductImageUploaderContent(handle: DocumentHandle) {
    ...
    <div className="space-y-4">
      ...
      {currentImages.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-200 py-8 dark:border-zinc-700">
          <ImageIcon className="mb-2 h-10 w-10 text-foreground/60" />
...
function ProductThumbnails({
  ...
  return (
    <div
      className={cn(
        "group relative aspect-square overflow-hidden rounded-lg bg-foreground/20",
        isFirst &&
    ...
    >
      ...
      {/* First image badge */}
      {isFirst && (
        <div className="absolute left-2 top-2 rounded bg-primary px-1.5 py-0.5 text-xs font-medium text-white">
          Main
        </div>
      )}
...
function ProductImageUploaderSkeleton() {
  return (
    <div className="space-y-4">
...
export function ProductImageUploader(props: DocumentHandle) {
  return (
    <Suspense fallback={<ProductImageUploaderSkeleton />}>
      <ProductImageUploaderContent {...props} />
    </Suspense>
  );
}
