...
export const businessProductType = defineType({
  ...
  fields: [
    defineField({
      ...
      readOnly: true,
      description: "Email of the owner who created this product",
    }),
    defineField({
      name: "businessName",
      title: "Business Name",
    ...
    defineField({
      ...
      initialValue: 10,
      description: "Business's profit margin percentage",
    }),
    defineField({
      name: "businessProductStatus",
      title: "Business Product Status",
      type: "string",
      options: {
        list: [
    ...
    defineField({
      ...
      initialValue: 0,
      readOnly: true,
      description: "Total profit earned by business",
    }),
    defineField({
      name: "lastOrderDate",
  ...
  preview: {
    ...
    prepare(selection) {
      const { title, business, status, media, price } = selection;
      const image = media && media[0];
      return {
        title: title,
        subtitle: `${business} - ${status} - $${price}`,
        media: image,
      };
    },
  },
