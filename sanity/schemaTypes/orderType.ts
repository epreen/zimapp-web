...
export const orderType = defineType({
  ...
  fields: [
        ...
        defineArrayMember({
          ...
          preview: {
            ...
            prepare({ title, quantity, price, media }) {
              return {
                title: title ?? "Product",
                subtitle: `Qty: ${quantity} • MWK ${price}`,
                media,
              };
            },
          },
    ...
    defineField({
      ...
      group: "details",
      readOnly: true,
      description: "Total order amount in MWK",
    }),
    defineField({
      name: "status",
      type: "string",
    ...
    defineField({
      ...
      group: "customer",
      readOnly: true,
      description: "Clerk user ID (copied from customer)",
    }),
    defineField({
      name: "email",
      type: "string",
    ...
    defineField({
      name: "paychanguPaymentId",
      type: "string",
      group: "payment",
      readOnly: true,
      description: "PayChangu payment ID",
    }),
    defineField({
      name: "createdAt",
      type: "datetime",
  ...
  preview: {
    ...
    prepare({ orderNumber, email, total, status }) {
      return {
        ...
        subtitle: `${email ?? "No email"} • MWK ${total ?? 0} • ${status ?? "paid"}`,
      };
    },
  },
  orderings: [
